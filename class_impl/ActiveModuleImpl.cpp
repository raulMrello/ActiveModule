...
State::StateResult ActiveModuleImpl::Init_EventHandler(State::StateEvent* se){
    ...
    switch((int)se->evt){
        case State::EV_ENTRY:{
        	int err = osOK;
        	DEBUG_TRACE("\r\nTemplImp\t Iniciando recuperaci�n de datos...");
        	...
        	if(err == osOK){
            	...
            	if(!checkIntegrity()){
            		DEBUG_TRACE("\r\nTemplImp\t ERR_CFG. Ha fallado el check de integridad. Establece configuraci�n por defecto.");
					setDefaultConfig();
            	}
				else{
					DEBUG_TRACE("\r\nTemplImp\t Recuperaci�n de datos OK!");
				}
        	}
			else{
				DEBUG_TRACE("\r\nTemplImp\t ERR_FS. Error en la recuperaci�n de datos. Establece configuraci�n por defecto");
				setDefaultConfig();
			}
        ...
        case WhichEvt:{
        	...
        	/* Si es necesario, almacena en el sistema de ficheros la configuraci�n o el par�metro correspondiente */
			...
			/* Si es necesario publica actualizaci�n en topic */
			//TODO
